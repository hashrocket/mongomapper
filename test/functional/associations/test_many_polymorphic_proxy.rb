...
class ManyPolymorphicProxyTest < Test::Unit::TestCase
  ...
  should "be able to replace the association" do
    ...
    messages = from_db.messages.all :sort => [["position", "asc"]]
  ...
  context "Finding scoped to association" do
    ...
    context "with :all" do
      should "work" do
        @lounge.messages.find(:all, :sort => [["position", "asc"]]).should == [@lm1, @lm2]
      end

      should "work with conditions" do
        messages = @lounge.messages.find(:all, :conditions => {:body => 'Loungin!'}, :sort => [["position", "asc"]])
        messages.should == [@lm1]
      end

      should "work with order" do
        messages = @lounge.messages.find(:all, :sort => [['position', 'desc']])
    ...
    context "with #all" do
      should "work" do
        @lounge.messages.all(:sort => [["position", "asc"]]).should == [@lm1, @lm2]
      end

      should "work with conditions" do
        messages = @lounge.messages.all(:conditions => {:body => 'Loungin!'}, :sort => [["position", "asc"]])
        messages.should == [@lm1]
      end

      should "work with order" do
        messages = @lounge.messages.all(:sort => [['position', 'desc']])
        messages.should == [@lm2, @lm1]
      end
    end
    ...
    context "with :first" do
      should "work" do
        @lounge.messages.find(:first, :sort => [["position", "asc"]]).should == @lm1
      end

      should "work with conditions" do
        message = @lounge.messages.find(:first, :conditions => {:body => 'I love loungin!'}, :sort => [["position", "asc"]])
        message.should == @lm2
      end
    end
    ...
    context "with #first" do
      should "work" do
        @lounge.messages.first(:sort => [["position", "asc"]]).should == @lm1
      end

      should "work with conditions" do
        message = @lounge.messages.first(:conditions => {:body => 'I love loungin!'}, :sort => [["position", "asc"]])
        message.should == @lm2
      end
    end
    ...
    context "with :last" do
      should "work" do
        @lounge.messages.find(:last, :sort => [["position", "asc"]]).should == @lm2
      end

      should "work with conditions" do
        message = @lounge.messages.find(:last, :conditions => {:body => 'Loungin!'}, :sort => [["position", "asc"]])
        message.should == @lm1
      end
    end
    ...
    context "with #last" do
      should "work" do
        @lounge.messages.last(:sort => [["position", "asc"]]).should == @lm2
      end

      should "work with conditions" do
        message = @lounge.messages.last(:conditions => {:body => 'Loungin!'}, :sort => [["position", "asc"]])
        message.should == @lm1
      end
    end
    ...
    context "with #paginate" do
      setup do
        @messages = @hall.messages.paginate(:per_page => 2, :page => 1, :sort => [['position', 'asc']])
      end
